...
namespace MrShen.Common
{
    internal class SummonerManager
    {
        ...
        private static string Smitetype
        {
            get
            {
                if (SmiteBlue.Any(i => EnsoulSharp.Common.Items.HasItem(i)))
                {
                    return "s5_summonersmiteplayerganker";
                }

                if (SmiteRed.Any(i => EnsoulSharp.Common.Items.HasItem(i)))
                {
                    return "s5_summonersmiteduel";
                }

                if (SmiteGrey.Any(i => EnsoulSharp.Common.Items.HasItem(i)))
                {
                    return "s5_summonersmitequick";
                }

                if (SmitePurple.Any(i => EnsoulSharp.Common.Items.HasItem(i)))
                {
                    return "itemsmiteaoe";
                }
        ...
        {
            var t = TargetSelector.GetTarget(MrShen.Champion.PlayerSpells.Q.Range, TargetSelector.DamageType.Magical);
            ...
            {
                if (ObjectManager.Player.GetSummonerSpellDamage(t, Damage.DamageSummonerSpell.Ignite) > t.Health
        ...
        private static void SmiteOnTarget(AIHeroClient t)
        {
            ...
            var itemCheck = SmiteBlue.Any(i => EnsoulSharp.Common.Items.HasItem(i)) || SmiteRed.Any(i => EnsoulSharp.Common.Items.HasItem(i));
            if (itemCheck && use &&
                ObjectManager.Player.Spellbook.CanUseSpell(SmiteSlot) == SpellState.Ready &&
